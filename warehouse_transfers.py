...
def create(from_warehouse:str=Form(...),to_warehouse:str=Form(...),lines_json:str=Form('[]'),notes:str=Form(''),x_access_code:str=Header(default='')):
 ...
 try:
  i=str(uuid.uuid4());n=no();c.execute('INSERT INTO warehouse_transfer_orders(id,transfer_no,from_warehouse,to_warehouse,status,notes,created_at) VALUES(?,?,?,?,?,?,?)',(i,n,from_warehouse,to_warehouse,'draft',notes,time.time()))
  for z in lines:
   sku=str(z.get('sku','')).strip();q=float(z.get('quantity',0))
   if not sku or q<=0:raise HTTPException(400,'Invalid transfer line')
   lid=str(uuid.uuid4());c.execute('INSERT INTO warehouse_transfer_lines(id,transfer_id,sku,requested_qty) VALUES(?,?,?,?)',(lid,i,sku,q));reserve_line(c,{'id':i,'from_warehouse':from_warehouse},{'id':lid,'sku':sku,'requested_qty':q})
