...
ALLOWED_PHOTO_TYPES={'image/jpeg','image/png','image/webp'};MAX_PHOTO_BYTES=15*1024*1024;router=APIRouter();TASK_STATUSES=['assigned','en_route_pickup','arrived_pickup','picked_up','en_route_dropoff','arrived_dropoff','dropped_off_customer','dropped_off_warehouse','completed','failed','cancelled'];PHOTO_REQUIRED_STATUSES={'picked_up','dropped_off_customer','dropped_off_warehouse'};DRIVER_STATUSES=['available','on_duty','off_duty'];VEHICLE_STATUSES=['available','in_use','maintenance'];FINAL_TASK_STATUSES={'completed','failed','cancelled','dropped_off_customer','dropped_off_warehouse'}
TASK_TRANSITIONS={'assigned':{'en_route_pickup','en_route_dropoff','cancelled','failed'},'en_route_pickup':{'arrived_pickup','cancelled','failed'},'arrived_pickup':{'picked_up','cancelled','failed'},'picked_up':{'en_route_dropoff','cancelled','failed'},'en_route_dropoff':{'arrived_dropoff','cancelled','failed'},'arrived_dropoff':{'dropped_off_customer','dropped_off_warehouse','cancelled','failed'},'dropped_off_customer':set(),'dropped_off_warehouse':set(),'completed':set(),'failed':set(),'cancelled':set()}
...
def has_active(c,column,value,exclude=''):
 if not value:return False
 q=f"SELECT 1 FROM dispatch_tasks WHERE {column}=? AND status NOT IN ('completed','failed','cancelled','dropped_off_customer','dropped_off_warehouse')";a=[value]
 if exclude:q+=' AND id!=?';a.append(exclude)
 return c.execute(q+' LIMIT 1',a).fetchone() is not None
def release_resources(c,r):
 if r['vehicle_id'] and not has_active(c,'vehicle_id',r['vehicle_id'],r['id']):c.execute("UPDATE vehicles SET status='available' WHERE id=? AND status!='maintenance'",(r['vehicle_id'],))
 if r['driver_id'] and not has_active(c,'driver_id',r['driver_id'],r['id']):c.execute("UPDATE drivers SET status='available' WHERE id=? AND is_active=1",(r['driver_id'],))
...
def vehicle_update(vehicle_id:str,plate_number:str=Form(None),vehicle_type:str=Form(None),capacity_kg:float=Form(None),status:str=Form(None),x_admin_passcode:str=Header(default='')):
 ...
 if status=='available' and has_active(c,'vehicle_id',vehicle_id):c.close();raise HTTPException(409,'Vehicle has an active dispatch task')
...
def vehicle_delete(vehicle_id:str,x_admin_passcode:str=Header(default='')):
 check_admin(x_admin_passcode);c=get_conn()
 if has_active(c,'vehicle_id',vehicle_id):c.close();raise HTTPException(409,'Vehicle has an active dispatch task')
 r=c.execute('DELETE FROM vehicles WHERE id=?',(vehicle_id,));c.commit();c.close();return {'id':vehicle_id,'deleted':bool(r.rowcount)}
...
def driver_update(driver_id:str,name:str=Form(None),phone:str=Form(None),vehicle_id:str=Form(None),is_active:bool=Form(None),x_admin_passcode:str=Header(default='')):
 ...
 if is_active is False and has_active(c,'driver_id',driver_id):c.close();raise HTTPException(409,'Driver has an active dispatch task')
...
def driver_delete(driver_id:str,x_admin_passcode:str=Header(default='')):
 check_admin(x_admin_passcode);c=get_conn()
 if has_active(c,'driver_id',driver_id):c.close();raise HTTPException(409,'Driver has an active dispatch task')
 r=c.execute('DELETE FROM drivers WHERE id=?',(driver_id,));c.commit();c.close();return {'id':driver_id,'deleted':bool(r.rowcount)}
...
def driver_status(status:str=Form(...),x_driver_passcode:str=Header(default='')):
 ...
 c=get_conn()
 if status!='on_duty' and has_active(c,'driver_id',d['id']):c.close();raise HTTPException(409,'Driver has an active dispatch task')
 c.execute('UPDATE drivers SET status=? WHERE id=?',(status,d['id']));c.commit();c.close();return {'status':status}
@router.get('/driver/tasks')
def my_tasks(x_driver_passcode:str=Header(default='')):
 d=require_driver(x_driver_passcode);c=get_conn();r=c.execute("SELECT * FROM dispatch_tasks WHERE driver_id=? AND status NOT IN ('completed','failed','cancelled','dropped_off_customer','dropped_off_warehouse') ORDER BY created_at",(d['id'],)).fetchall();c.close();return {'count':len(r),'results':[dict(x) for x in r]}
...
def create_dispatch(task_type:str=Form(...),location_text:str=Form(...),latitude:float=Form(None),longitude:float=Form(None),shipment_number:str=Form(''),driver_id:str=Form(''),vehicle_id:str=Form(''),notes:str=Form(''),scheduled_at:float=Form(None),x_admin_passcode:str=Header(default='')):
 ...
 if vehicle_id:
  ...
  if v['status']!='available' or has_active(c,'vehicle_id',vehicle_id):c.close();raise HTTPException(409,'Vehicle is not available')
 if driver_id:
  d=c.execute('SELECT * FROM drivers WHERE id=? AND is_active=1',(driver_id,)).fetchone()
  if not d:c.close();raise HTTPException(404,'Active driver not found')
  if d['status']!='available' or has_active(c,'driver_id',driver_id):c.close();raise HTTPException(409,'Driver is not available')
 tid=str(uuid.uuid4());num=next_task();c.execute('INSERT INTO dispatch_tasks (id,task_number,shipment_number,task_type,location_text,latitude,longitude,driver_id,vehicle_id,status,notes,scheduled_at,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',(tid,num,shipment_number or None,task_type,location_text,latitude,longitude,driver_id or None,vehicle_id or None,'assigned',notes,scheduled_at,time.time()));log(c,tid,'assigned','Task created')
 if vehicle_id:c.execute("UPDATE vehicles SET status='in_use' WHERE id=?",(vehicle_id,))
 if driver_id:c.execute("UPDATE drivers SET status='on_duty' WHERE id=?",(driver_id,))
 c.commit();c.close();return {'id':tid,'task_number':num,'status':'assigned'}
...
def assign(task_id:str,driver_id:str=Form(''),vehicle_id:str=Form(''),x_admin_passcode:str=Header(default='')):
 ...
 if r['status'] in FINAL_TASK_STATUSES:c.close();raise HTTPException(409,'Completed or closed tasks cannot be reassigned')
 if vehicle_id and vehicle_id!=r['vehicle_id']:
  v=c.execute('SELECT * FROM vehicles WHERE id=?',(vehicle_id,)).fetchone()
  if not v:c.close();raise HTTPException(404,'Vehicle not found')
  if v['status']!='available' or has_active(c,'vehicle_id',vehicle_id,task_id):c.close();raise HTTPException(409,'Vehicle is not available')
 if driver_id and driver_id!=r['driver_id']:
  d=c.execute('SELECT * FROM drivers WHERE id=? AND is_active=1',(driver_id,)).fetchone()
  if not d:c.close();raise HTTPException(404,'Active driver not found')
  if d['status']!='available' or has_active(c,'driver_id',driver_id,task_id):c.close();raise HTTPException(409,'Driver is not available')
 oldv=r['vehicle_id'];oldd=r['driver_id'];c.execute('UPDATE dispatch_tasks SET driver_id=?,vehicle_id=? WHERE id=?',(driver_id or None,vehicle_id or None,task_id));log(c,task_id,r['status'],'Reassigned')
 if vehicle_id:c.execute("UPDATE vehicles SET status='in_use' WHERE id=?",(vehicle_id,))
 if driver_id:c.execute("UPDATE drivers SET status='on_duty' WHERE id=?",(driver_id,))
 nr=dict(r);nr['vehicle_id']=oldv;nr['driver_id']=oldd;release_resources(c,nr);c.commit();c.close();return {'id':task_id,'reassigned':True}
...
async def task_status(task_id:str,status:str=Form(...),note:str=Form(''),photo:UploadFile=File(None),x_driver_passcode:str=Header(default=''),x_admin_passcode:str=Header(default='')):
 ...
 if status==r['status']:c.close();return {'id':task_id,'status':status,'unchanged':True}
 allowed=TASK_TRANSITIONS.get(r['status'],set())
 if status not in allowed:c.close();raise HTTPException(409,f"Cannot move task from {r['status']} to {status}")
 if r['task_type']=='dropoff_customer' and status=='dropped_off_warehouse':c.close();raise HTTPException(409,'Customer delivery task cannot complete at warehouse')
 if r['task_type'] in {'dropoff_warehouse','warehouse_transfer'} and status=='dropped_off_customer':c.close();raise HTTPException(409,'Warehouse task cannot complete as customer delivery')
 if status in PHOTO_REQUIRED_STATUSES and (photo is None or not photo.filename):c.close();raise HTTPException(400,f"A photo is required to mark this task as '{status.replace('_',' ')}'")
 photo_url=None
 ...
 completed=status in FINAL_TASK_STATUSES;now=time.time();c.execute('UPDATE dispatch_tasks SET status=?,photo_url=COALESCE(?,photo_url),completed_at=? WHERE id=?',(status,photo_url,now if completed else r['completed_at'],task_id));log(c,task_id,status,note,photo_url);synchronized={}
 if completed:
  release_resources(c,r);synchronized['resources']='released_if_idle'
  order=None
  ...
  if not order:
   y=c.execute("SELECT order_number FROM yard_units WHERE notes LIKE ? ORDER BY updated_at DESC LIMIT 1",('%'+str(r['task_number'])+'%',)).fetchone()
   if y and y['order_number']:order=c.execute('SELECT * FROM orders WHERE order_number=?',(y['order_number'],)).fetchone()
  if order and status=='dropped_off_customer' and order['status']=='shipped':
  ...
  if r['shipment_number'] and status=='dropped_off_customer':
   try:
    cols={x['name'] for x in c.execute('PRAGMA table_info(shipments)').fetchall()}
    if 'delivery_status' in cols:c.execute("UPDATE shipments SET delivery_status='delivered' WHERE shipment_number=?",(r['shipment_number'],));synchronized['shipment']=r['shipment_number']
