"""Readiness, live-sync and freshness diagnostics for UGAMAP/UGASHIP backup."""
...
def backup_status():
 ...
 try:
  with urllib.request.urlopen(f"{BACKUP_SERVICE_URL}/health",timeout=4) as response:
   remote=json.loads(response.read().decode());reachable=200<=response.status<300;db_ok=remote.get("database")=="connected"
 except Exception as exc:error=f"{type(exc).__name__}: {exc}"[:180]
 client=sync_client_status();last=client.get("last_success");age=_age(last)
 fresh=age is not None and age<=MAX_BACKUP_AGE_SECONDS
 records=(remote or {}).get("records",{})
 return {"backup_enabled":token,"backup_service_reachable":reachable,"backup_database_connected":db_ok,"sync_ready":token and reachable and db_ok,"backup_fresh":fresh,"max_backup_age_seconds":MAX_BACKUP_AGE_SECONDS,"last_sync_age_seconds":round(age,1) if age is not None else None,"backup_counts":{"UGAMAP":records.get("UGAMAP"),"UGASHIP":records.get("UGASHIP"),"deleted":records.get("deleted"),"snapshots":records.get("snapshots")},"backup_service":BACKUP_SERVICE_URL,"checked_at":datetime.now(timezone.utc).isoformat(),"last_sync_attempt":client.get("last_attempt"),"last_sync_success":last,"last_sync_error":client.get("last_error"),"remote_last_activity":(remote or {}).get("last_activity"),"remote_health":remote,"error":error}
