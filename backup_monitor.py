...
def backup_status():
 ...
 client=sync_client_status();reconcile=reconcile_status();ugamap=_freshness(reconcile.get("UGAMAP"));ugaship=_freshness(reconcile.get("UGASHIP"));live_sources={"UGAMAP":_live_freshness(client.get("UGAMAP")),"UGASHIP":_live_freshness(client.get("UGASHIP"))};fresh=ugamap["fresh"] and ugaship["fresh"];records=(remote or {}).get("records",{});queue={"queued":client.get("queued",0),"active":client.get("active",0),"completed":client.get("completed",0),"failed":client.get("failed",0),"dropped":client.get("dropped",0),"max_workers":client.get("max_workers"),"max_outstanding":client.get("max_outstanding")}
