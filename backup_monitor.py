...
def backup_status():
 ...
 client=sync_client_status();reconcile=reconcile_status();ugamap=_freshness(reconcile.get("UGAMAP"));ugaship=_freshness(reconcile.get("UGASHIP"));warehouse=_freshness(reconcile.get("WAREHOUSE"));warehouse["tables"]=reconcile.get("WAREHOUSE",{}).get("tables");live_sources={"UGAMAP":_live_freshness(client.get("UGAMAP")),"UGASHIP":_live_freshness(client.get("UGASHIP")),"WAREHOUSE":_live_freshness(client.get("WAREHOUSE"))};fresh=ugamap["fresh"] and ugaship["fresh"] and warehouse["fresh"];ready=token and reachable and db_ok;records=(remote or {}).get("records",{});queue={"queued":client.get("queued",0),"active":client.get("active",0),"completed":client.get("completed",0),"failed":client.get("failed",0),"dropped":client.get("dropped",0),"max_workers":client.get("max_workers"),"max_outstanding":client.get("max_outstanding")}
 reasons=[]
 ...
 if not warehouse["fresh"]:reasons.append("Warehouse reconciliation stale or unhealthy")
