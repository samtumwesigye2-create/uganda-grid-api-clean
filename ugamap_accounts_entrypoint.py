...
for route in list(app.router.routes):
    ...
    if path == "/report" and "POST" in methods:
        app.router.routes.remove(route)
    if path == "/reports/{report_id}/confirm" and "POST" in methods:
        app.router.routes.remove(route)
...
async def account_report(
    category:str=Form(...), lat:float=Form(...), lon:float=Form(...), note:str=Form(""),
    file:UploadFile=File(None), authorization:str=Header(default="")
):
    ...
    report["submitted_by_account"]=True
    return report
...
def account_my_reports(authorization:str=Header(default="")):
    user=_account_user(authorization)
    rows=list_user_reports(user["id"])
