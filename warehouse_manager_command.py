...
from vector5250_executor import router as vector_executor_router

router=APIRouter(tags=['Vector 5250'])
for r in (vector_records_router,vector_resilience_router,vector_jobs_router,vector_queues_router,vector_messages_router,vector_scheduler_router,vector_profiles_router,vector_executor_router):router.include_router(r)
...
def _manager(access_code:str):
    ...
    try:require_permission(access_code,MANAGER_PERMISSION)
        ...
        if exc.status_code in (401,403):raise HTTPException(status_code=403,detail='Vector Manager or higher access required')
        raise
    return {'role':'vector_manager','manager_access':True}
...
@router.get('/vector5250-business.js',include_in_schema=False)
def businessjs():return js('vector5250-business.js')
