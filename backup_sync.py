...
def _send(payload):
    _change("queued",-1);_change("active",1);success=False
    try:
        if not BACKUP_SYNC_TOKEN:
            _set(last_error="BACKUP_SYNC_TOKEN not configured");return
            ...
            try:
                ...
                with urllib.request.urlopen(req,timeout=6) as response:
                    ...
                    if 200<=response.status<300:
                        success=True;_set(last_success=datetime.now(timezone.utc).isoformat(),last_error=None);return
