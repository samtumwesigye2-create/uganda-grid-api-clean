...
def _valhalla_request(body: bytes):
    return requests.post(VALHALLA_URL, data=body, headers={"Content-Type":"application/json","Accept":"application/json","User-Agent":"UGAMAP/1.0"}, timeout=12)
...
def _osrm_fallback(body: bytes):
    ...
    r = requests.get(f"{OSRM_BASE}/{coords}", params={"overview":"full","geometries":"polyline6","steps":"false"}, headers={"Accept":"application/json","User-Agent":"UGAMAP/1.0"}, timeout=12)
    r.raise_for_status()
    data = r.json()
...
async def _route_response(body: bytes):
    if not body:
        return Response('{"error":"missing route payload"}', status_code=400, media_type="application/json", headers={"Cache-Control":"no-store"})
    try:
...
async def _routing_test():
    # Kampala -> Entebbe. Safe GET diagnostic that exercises the same backend engines.
    payload = json.dumps({"locations":[{"lat":0.3476,"lon":32.5825},{"lat":0.0512,"lon":32.4637}],"costing":"auto","units":"kilometers"}).encode("utf-8")
    response = await _route_response(payload)
    ok = response.status_code == 200
    router = response.headers.get("X-UGAMAP-Router", "none")
    return Response(json.dumps({"release":RELEASE,"routing_test":"PASS" if ok else "FAIL","router":router,"status_code":response.status_code}), status_code=200 if ok else 502, media_type="application/json", headers={"Cache-Control":"no-store"})
    ...
    try:
        if scope.get("type") == "http":
            ...
            if method == "POST" and path == "/routing/route":
                response = await _route_response(await _read_body(receive))
            ...
            if method == "GET":
                ...
                if path == "/system/release":
                    response = Response(json.dumps({"release":RELEASE,"routing_proxy":True,"routing_fallback":"osrm"}), media_type="application/json", headers={"Cache-Control":"no-store"})
                    await response(scope, receive, relay_send); return
                if path == "/routing/test":
                    response = await _routing_test()
                    await response(scope, receive, relay_send); return
        ...
        if scope.get("type") == "http":
            relay_emit("api_call", "error" if status_code >= 500 else "warning" if status_code >= 400 else "info", {"method":scope.get("method"),"path":scope.get("path"),"status_code":status_code,"duration_ms":round((time.perf_counter()-started)*1000,2),"service":"ugamap"})
