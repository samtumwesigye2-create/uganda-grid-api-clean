...
def available_codes(category):
    ...
    used={x.get("zip_code") for x in _load()};result=[];anchor=_anchor_for_category(category)
...
def create_assignment(category,name,latitude,longitude,address="",notes="",zip_code=None):
    ...
    runtime=_runtime_load();runtime.append(item);_save(runtime);return item
...
def update_assignment(old_zip_code,category=None,name=None,latitude=None,longitude=None,address=None,notes=None,zip_code=None):
    ...
    runtime=[x for x in _runtime_load() if str(x.get("zip_code","")).zfill(5)!=old];runtime.append(updated);_save(runtime);return updated
...
def delete_assignment(zip_code):
    ...
    runtime=_runtime_load();new=[x for x in runtime if str(x.get("zip_code","")).zfill(5)!=code]
    if len(new)==len(runtime):return False
    _save(new);return True

def category_catalog():return [{"anchor":anchor,**info,"range":SPECIAL_BLOCKS[info["category"]]} for anchor,info in SPECIAL_CATEGORY_ANCHORS.items()]
