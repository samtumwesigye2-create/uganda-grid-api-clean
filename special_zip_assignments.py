...
survives restarts, redeploys, and application updates. Existing JSON assignments
are migrated automatically without overwriting newer database records.
"""
import json, os
...
DEFAULT_ASSIGNMENTS=[{"zip_code":"00001","category":"state_house","name":"State House Uganda","latitude":0.05987,"longitude":32.46913,"address":"Entebbe","notes":"No fly zone","special":True,"locked_anchor":True}]
 ...
 try:
  with conn:
   with conn.cursor() as cur:
    cur.execute("""CREATE TABLE IF NOT EXISTS special_zip_assignments(
    zip_code VARCHAR(5) PRIMARY KEY,category VARCHAR(64) NOT NULL,name TEXT NOT NULL,
    latitude DOUBLE PRECISION NOT NULL,longitude DOUBLE PRECISION NOT NULL,address TEXT NOT NULL DEFAULT '',
    notes TEXT NOT NULL DEFAULT '',special BOOLEAN NOT NULL DEFAULT TRUE,locked_anchor BOOLEAN NOT NULL DEFAULT FALSE,
...
def _load_db():
 ...
 try:
  with conn.cursor() as cur:
   ...
   return [{"zip_code":r[0],"category":r[1],"name":r[2],"latitude":r[3],"longitude":r[4],"address":r[5],"notes":r[6],"special":r[7],"locked_anchor":r[8],"persistent":True} for r in cur.fetchall()]
 finally:conn.close()
def _upsert_db(item,overwrite=True):
 _init_db();conn=_connect()
 if not conn:return False
 try:
  with conn:
   with conn.cursor() as cur:
    if overwrite:
     ...
     cur.execute("""INSERT INTO special_zip_assignments(zip_code,category,name,latitude,longitude,address,notes,special,locked_anchor)
     VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT(zip_code) DO NOTHING""",
     (item["zip_code"],item["category"],item["name"],item["latitude"],item["longitude"],item.get("address",""),item.get("notes",""),True,bool(item.get("locked_anchor",False))))
  return True
 finally:conn.close()
def _delete_db(code):
 ...
 try:
  with conn:
   with conn.cursor() as cur:cur.execute("DELETE FROM special_zip_assignments WHERE zip_code=%s",(code,));return cur.rowcount>0
 finally:conn.close()
def _migrate_json_to_db():
 if not DATABASE_URL:return 0
 moved=0
 for item in _runtime_load_json():
  try:
   item=dict(item);item["zip_code"]=str(item.get("zip_code","")).zfill(5)
   if item.get("zip_code") and item.get("category") and item.get("name"):_upsert_db(item,overwrite=False);moved+=1
  except Exception as e:print("Special ZIP migration item unavailable:",e)
 return moved
def _load():
 merged={x["zip_code"]:{**x,"persistent":True if DATABASE_URL else False} for x in DEFAULT_ASSIGNMENTS}
 try:
  if DATABASE_URL:_migrate_json_to_db();runtime=_load_db()
  else:runtime=_runtime_load_json()
 except Exception as e:print("Special ZIP database read unavailable:",e);runtime=_runtime_load_json()
 for item in runtime:
...
def valid_code_for_category(category,code):
 value=str(code or "").zfill(5);return value==_anchor_for_category(category) or value in _legacy_codes(category) or value in _block_codes(category)
def available_codes(category):
 ...
 result.extend(z for z in _legacy_codes(category) if z not in used);result.extend(z for z in _block_codes(category) if z not in used);return result
def next_code(category):
 codes=available_codes(category);return codes[0] if codes else None
def _persist_item(item):
 item={**item,"persistent":bool(DATABASE_URL)}
 if DATABASE_URL:
  if not _upsert_db(item,overwrite=True):raise ValueError("Persistent database unavailable")
 else:
...
def create_assignment(category,name,latitude,longitude,address="",notes="",zip_code=None):
 ...
 item={"zip_code":code,"category":category,"name":name,"latitude":float(latitude),"longitude":float(longitude),"address":str(address or "").strip(),"notes":str(notes or "").strip(),"special":True,"persistent":bool(DATABASE_URL)}
