...
def configure_core(*, address_source, state_lookup=None, zip_lookup=None, reports_source=None,
                   search_source=None, address_lookup=None, location_lookup=None) -> None:
...
def _normalized_address(record):
    return {
        ...
        "zip_code": record.get("zip_code"), "state_code": record.get("state_code"),
...
@router.get("/route")
def core_route(start_lat: float, start_lon: float, dest_lat: float, dest_lon: float,
               mode: str=Query("driving",pattern="^(driving|walking|cycling|flight)$")):
    if mode == "flight":
        distance=_haversine_m(start_lat,start_lon,dest_lat,dest_lon)
        return {"mode":mode,"points":[[start_lat,start_lon],[dest_lat,dest_lon]],"distance_m":distance,
                "duration_s":distance/800000*3600,"maneuvers":[],"provider":"direct"}
    costing={"driving":"auto","walking":"pedestrian","cycling":"bicycle"}[mode]
    payload={"locations":[{"lat":start_lat,"lon":start_lon},{"lat":dest_lat,"lon":dest_lon}],"costing":costing,"units":"kilometers"}
    try:
        response=requests.post("https://valhalla1.openstreetmap.de/route",json=payload,timeout=10)
        response.raise_for_status(); data=response.json()
    except (requests.RequestException,ValueError) as exc:
        raise HTTPException(status_code=503,detail="Routing service is temporarily unavailable") from exc
