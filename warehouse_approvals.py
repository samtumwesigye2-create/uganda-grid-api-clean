...
def init():
 c=db();c.execute('''CREATE TABLE IF NOT EXISTS warehouse_approval_requests(id TEXT PRIMARY KEY,request_no TEXT UNIQUE NOT NULL,warehouse_id TEXT NOT NULL,action_type TEXT NOT NULL,reference_no TEXT,requested_by TEXT NOT NULL,requested_by_code TEXT,reason TEXT,payload_json TEXT,status TEXT NOT NULL DEFAULT 'pending',approved_by TEXT,decision_note TEXT,created_at REAL NOT NULL,decided_at REAL)''');c.execute('''CREATE TABLE IF NOT EXISTS warehouse_security_audit(id TEXT PRIMARY KEY,event_no TEXT UNIQUE NOT NULL,warehouse_id TEXT NOT NULL,event_type TEXT NOT NULL,action_type TEXT,reference_no TEXT,actor TEXT,approval_id TEXT,details TEXT,created_at REAL NOT NULL)''');c.execute('CREATE INDEX IF NOT EXISTS idx_approval_queue ON warehouse_approval_requests(warehouse_id,status,created_at)');c.execute('CREATE INDEX IF NOT EXISTS idx_security_audit ON warehouse_security_audit(warehouse_id,created_at)');c.commit();c.close()
init()
...
SENSITIVE={'inventory_adjustment','stock_writeoff','destroy_inventory','dispatch_release','recall_close','quality_release','transfer_ship','purchase_approval'}
...
def decision(approval_id:str,decision:str=Form(...),approved_by:str=Form(...),decision_note:str=Form(''),x_access_code:str=Header(default='')):
 ...
 c=db();r=c.execute('SELECT * FROM warehouse_approval_requests WHERE id=?',(approval_id,)).fetchone()
 if not r:c.close();raise HTTPException(404,'Approval request not found')
 if r['status']!='pending':c.close();raise HTTPException(400,'Approval already decided')
 if approved_by.strip().lower()==r['requested_by'].strip().lower():c.close();raise HTTPException(403,'Separation of duties: requester cannot approve own action')
 c.execute('UPDATE warehouse_approval_requests SET status=?,approved_by=?,decision_note=?,decided_at=? WHERE id=?',(decision,approved_by,decision_note,time.time(),approval_id));audit(c,r['warehouse_id'],'approval_decision',r['action_type'],r['reference_no'],approved_by,approval_id,decision+': '+decision_note);c.commit();c.close();return {'id':approval_id,'status':decision}
...
def authorize(approval_id:str,action_type:str=Query(...),reference_no:str=Query(''),x_access_code:str=Header(default='')):
 auth(x_access_code,'inventory:write');c=db();r=c.execute('SELECT * FROM warehouse_approval_requests WHERE id=?',(approval_id,)).fetchone();ok=bool(r and r['status']=='approved' and r['action_type']==action_type and (not reference_no or r['reference_no']==reference_no));c.close();return {'authorized':ok,'approval_id':approval_id}
...
def log(event_type:str=Form(...),action_type:str=Form(''),warehouse_id:str=Form('main'),reference_no:str=Form(''),actor:str=Form(''),approval_id:str=Form(''),details:str=Form(''),x_access_code:str=Header(default='')):
 auth(x_access_code,'inventory:write');c=db();audit(c,warehouse_id,event_type,action_type,reference_no,actor,approval_id,details);c.commit();c.close();return {'logged':True}
