...
def init_db():
 c=conn();c.executescript('''CREATE TABLE IF NOT EXISTS warehouse_suppliers(id TEXT PRIMARY KEY,supplier_code TEXT NOT NULL UNIQUE,name TEXT NOT NULL,contact_name TEXT,email TEXT,phone TEXT,address TEXT,status TEXT NOT NULL DEFAULT 'active',created_at REAL NOT NULL,updated_at REAL NOT NULL);CREATE TABLE IF NOT EXISTS warehouse_purchase_orders(id TEXT PRIMARY KEY,po_number TEXT NOT NULL UNIQUE,supplier_id TEXT NOT NULL,warehouse_id TEXT NOT NULL DEFAULT 'main',expected_date TEXT,status TEXT NOT NULL DEFAULT 'open',notes TEXT,created_at REAL NOT NULL,updated_at REAL NOT NULL);CREATE TABLE IF NOT EXISTS warehouse_po_lines(id TEXT PRIMARY KEY,po_id TEXT NOT NULL,sku TEXT NOT NULL,description TEXT,ordered_qty REAL NOT NULL DEFAULT 0,received_qty REAL NOT NULL DEFAULT 0,uom TEXT NOT NULL DEFAULT 'each');CREATE TABLE IF NOT EXISTS warehouse_asn(id TEXT PRIMARY KEY,asn_number TEXT NOT NULL UNIQUE,po_id TEXT NOT NULL,carrier TEXT,tracking_no TEXT,eta TEXT,status TEXT NOT NULL DEFAULT 'expected',notes TEXT,created_at REAL NOT NULL,updated_at REAL NOT NULL);CREATE TABLE IF NOT EXISTS warehouse_po_receipts(id TEXT PRIMARY KEY,receipt_key TEXT NOT NULL UNIQUE,po_id TEXT NOT NULL,po_line_id TEXT NOT NULL,sku TEXT NOT NULL,quantity REAL NOT NULL,warehouse_operation_id TEXT,received_at REAL NOT NULL);''');c.commit();c.close()
init_db()
...
def create_po(supplier_id:str=Form(...),warehouse_id:str=Form('main'),expected_date:str=Form(''),notes:str=Form(''),lines_json:str=Form('[]'),x_access_code:str=Header(default='')):
 ...
 for x in lines:
  ...
  if sku and qty>0:c.execute('INSERT INTO warehouse_po_lines VALUES(?,?,?,?,?,?,?)',(str(uuid.uuid4()),pid,sku,str(x.get('description','')),qty,0,str(x.get('uom','each'))))
 ...
 c.commit();r=dict(c.execute('SELECT * FROM warehouse_purchase_orders WHERE id=?',(pid,)).fetchone());c.close();return r
...
def purchase_orders(status:str=Query('open'),x_access_code:str=Header(default='')):
 ...
 for r in rows:d=dict(r);d['lines']=[dict(x) for x in c.execute('SELECT * FROM warehouse_po_lines WHERE po_id=?',(r['id'],)).fetchall()];out.append(d)
...
def receive_po(po_id:str,sku:str=Form(...),quantity:float=Form(...),receipt_key:str=Form(''),warehouse_operation_id:str=Form(''),x_access_code:str=Header(default='')):
 require_permission(x_access_code,'inventory:write')
 if quantity<=0:raise HTTPException(400,'Received quantity must be greater than zero')
 c=conn();c.execute('BEGIN IMMEDIATE')
 try:
  po=c.execute('SELECT * FROM warehouse_purchase_orders WHERE id=?',(po_id,)).fetchone();line=c.execute('SELECT * FROM warehouse_po_lines WHERE po_id=? AND sku=?',(po_id,sku)).fetchone()
  if not po or not line:raise HTTPException(404,'SKU is not on this purchase order')
  key=(receipt_key.strip() or warehouse_operation_id.strip())
  if not key:raise HTTPException(400,'receipt_key or warehouse_operation_id is required for idempotent receiving')
  old=c.execute('SELECT * FROM warehouse_po_receipts WHERE receipt_key=?',(key,)).fetchone()
  if old:c.rollback();c.close();return {'po_id':old['po_id'],'sku':old['sku'],'received_now':old['quantity'],'idempotent':True}
  remaining=float(line['ordered_qty'])-float(line['received_qty'])
  if quantity>remaining:raise HTTPException(409,f'Over-receipt blocked. Remaining PO quantity is {remaining}')
  new=float(line['received_qty'])+quantity;c.execute('UPDATE warehouse_po_lines SET received_qty=? WHERE id=?',(new,line['id']));c.execute('INSERT INTO warehouse_po_receipts VALUES(?,?,?,?,?,?,?)',(str(uuid.uuid4()),key,po_id,line['id'],sku,quantity,warehouse_operation_id or None,time.time()));unreceived=c.execute('SELECT COUNT(*) n FROM warehouse_po_lines WHERE po_id=? AND received_qty<ordered_qty',(po_id,)).fetchone()['n'];status='received' if unreceived==0 else 'partial';c.execute('UPDATE warehouse_purchase_orders SET status=?,updated_at=? WHERE id=?',(status,time.time(),po_id));c.commit();return {'po_id':po_id,'po_number':po['po_number'],'sku':sku,'received_now':quantity,'received_total':new,'remaining':float(line['ordered_qty'])-new,'po_status':status,'idempotent':False}
