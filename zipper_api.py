...
def zipper_feature_collection():
    if os.path.exists(ZIPPER_GEOJSON_FILE): return _load_artifact()
    return live_zipper_feature_collection()

def clear_zipper_cache():
    _load_artifact.cache_clear(); live_zipper_feature_collection.cache_clear()
...
def geography_zipper_status():
    status=numbering_status(); artifact_ready=os.path.exists(ZIPPER_GEOJSON_FILE)
    status.update({"layer":"ZIPPER","active_replacement":True,"artifact":os.path.basename(ZIPPER_GEOJSON_FILE),"artifact_ready":artifact_ready,"source":"generated_artifact" if artifact_ready else "district_population_live_fallback"})
    try:
        if artifact_ready:
            fc=_load_artifact(); status["zones"]=len(fc.get("features",[])); status["ready"]=True
        else:status.update(live_zipper_status())
    except Exception as exc:
        status["ready"]=False; status["error"]=str(exc)
    return status
