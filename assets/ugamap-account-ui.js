(() => {
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='ugAcctStyle';s.textContent=`
      #ugAcctBtn,#ugAcctMoreBtn{display:none!important}
      #ugAcctMenu{position:fixed;left:10px;top:52px;z-index:11000;width:min(280px,calc(100vw - 20px));display:none;background:#0f172a;color:#e5e7eb;border:1px solid #334155;border-radius:12px;padding:8px;box-shadow:0 12px 30px #0009;font-family:system-ui}
      #ugAcctMenu.open{display:block} #ugAcctMenu button{display:block;width:100%;text-align:left;padding:12px;border:0;border-radius:9px;background:transparent;color:#fff;font-weight:650;font-size:14px} #ugAcctMenu button:active{background:#1e293b}
      ...
      #ugAcctMsg{font-size:13px;margin:8px 0;min-height:18px}.ugAcctTabs{display:flex;gap:8px;margin-bottom:10px}.ugAcctTabs button{flex:1}.ugAcctSection{display:none}.ugAcctSection.active{display:block}.ugAcctSmall{font-size:12px;color:#94a3b8}
    `;document.head.appendChild(s);
  }

  function installHamburgerMenu(){
    const oldMore=document.getElementById('ugAcctMoreBtn');if(oldMore)oldMore.remove();
    const oldFloat=document.getElementById('ugAcctBtn');if(oldFloat)oldFloat.remove();
    const menuBtn=document.getElementById('menuBtn');if(!menuBtn)return;
    let menu=document.getElementById('ugAcctMenu');
    if(!menu){menu=document.createElement('div');menu.id='ugAcctMenu';menu.innerHTML='<button id="ugAcctMenuAccount" type="button">👤 Create Account / Login</button>';document.body.appendChild(menu);}
    menuBtn.onclick=(e)=>{e.stopPropagation();menu.classList.toggle('open');};
    document.getElementById('ugAcctMenuAccount').onclick=()=>{menu.classList.remove('open');open();};
  ...
  function ensureUI(){
    ...
    const m=document.createElement('div');m.id='ugAcctModal';m.innerHTML=`<div id="ugAcctCard">
      ...
      <div id="ugAcctMsg"></div><div id="ugAcctGuest"><div class="ugAcctTabs"><button type="button" data-tab="login">Login</button><button type="button" class="secondary" data-tab="signup">Sign Up</button></div>
      ...
      <section id="ugSignup" class="ugAcctSection"><input id="ugSignupEmail" type="email" autocomplete="email" placeholder="Email"><input id="ugSignupPhone" type="tel" autocomplete="tel" placeholder="Phone number"><input id="ugSignupAddress" type="text" autocomplete="street-address" placeholder="Address"><input id="ugSignupPassword" type="password" autocomplete="new-password" placeholder="Password (8+ characters)"><button id="ugSignupBtn" type="button">Create Account</button></section></div>
      <div id="ugAcctProfile" style="display:none"><div class="ugAcctSmall">Signed in as</div><div id="ugAcctIdentity" style="font-weight:700;margin-bottom:10px"></div><input id="ugProfileEmail" type="email" placeholder="Email"><input id="ugProfilePhone" type="tel" placeholder="Phone number"><input id="ugProfileAddress" type="text" placeholder="Address"><button id="ugSaveProfile" type="button">Save Profile</button><hr style="border:0;border-top:1px solid #334155;margin:16px 0"><div class="ugAcctSmall">Change password</div><input id="ugCurrentPassword" type="password" placeholder="Current password"><input id="ugNewPassword" type="password" placeholder="New password"><button id="ugChangePassword" type="button">Change Password</button><hr style="border:0;border-top:1px solid #334155;margin:16px 0"><button id="ugLogout" class="danger" type="button">Logout</button></div></div>`;
    document.body.appendChild(m);m.addEventListener('click',e=>{if(e.target===m)close();});document.getElementById('ugAcctClose').onclick=close;
    m.querySelectorAll('[data-tab]').forEach(btn=>btn.onclick=()=>{const loginTab=btn.dataset.tab==='login';document.getElementById('ugLogin').classList.toggle('active',loginTab);document.getElementById('ugSignup').classList.toggle('active',!loginTab);m.querySelectorAll('[data-tab]').forEach(x=>x.classList.toggle('secondary',x!==btn));setMsg('');});
    document.getElementById('ugLoginBtn').onclick=login;document.getElementById('ugSignupBtn').onclick=signup;document.getElementById('ugSaveProfile').onclick=saveProfile;document.getElementById('ugChangePassword').onclick=changePassword;document.getElementById('ugLogout').onclick=logout;
  }
  ...
  function render(){const guest=document.getElementById('ugAcctGuest'),profile=document.getElementById('ugAcctProfile'),menu=document.getElementById('ugAcctMenuAccount');if(!guest)return;const signed=!!currentUser;guest.style.display=signed?'none':'block';profile.style.display=signed?'block':'none';if(menu)menu.textContent=signed?'👤 Account / Profile':'👤 Create Account / Login';if(signed){document.getElementById('ugAcctIdentity').textContent=currentUser.email||currentUser.id;document.getElementById('ugProfileEmail').value=currentUser.email||'';document.getElementById('ugProfilePhone').value=currentUser.phone||'';document.getElementById('ugProfileAddress').value=currentUser.address||'';}}
  ...
  function loadLanguageLayer(){if(document.getElementById('ugLangScript'))return;const s=document.createElement('script');s.id='ugLangScript';s.src='/assets/ugamap-language-layer.js?v=1';s.defer=true;document.head.appendChild(s);}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{ensureUI();refreshUser();loadLanguageLayer();});else{ensureUI();refreshUser();loadLanguageLayer();}
})();