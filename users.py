"""User account registration, login, session, profile, password and UGATU role management."""
import hashlib,hmac,os,re,secrets,sqlite3,time
from fastapi import APIRouter,Header,HTTPException,Request
from pydantic import BaseModel
from ugatu_auth import ROLE_RULES,normalize_role
...
def init_db():
 c=get_conn()
 c.execute('CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY,name TEXT NOT NULL,email TEXT UNIQUE NOT NULL,password_hash TEXT NOT NULL,password_salt TEXT NOT NULL,phone TEXT,address TEXT,created_at REAL NOT NULL)')
 cols={r['name'] for r in c.execute('PRAGMA table_info(users)').fetchall()}
 if 'ugatu_role' not in cols:c.execute("ALTER TABLE users ADD COLUMN ugatu_role TEXT NOT NULL DEFAULT 'customer'")
 c.execute('CREATE TABLE IF NOT EXISTS sessions (token TEXT PRIMARY KEY,user_id TEXT NOT NULL,created_at REAL NOT NULL)')
 c.commit();c.close()
init_db()
...
def user_to_dict(r):return {'id':r['id'],'name':r['name'],'email':r['email'],'phone':r['phone'] or '','address':r['address'] or '','ugatu_role':r['ugatu_role'] or 'customer'}
...
class AdminUGATURoleBody(BaseModel):email:str;role:str
...
def signup(body:SignupBody):
 ...
 ph,salt=hash_password(body.password);uid=secrets.token_hex(16)
 c.execute('INSERT INTO users (id,name,email,password_hash,password_salt,phone,address,created_at,ugatu_role) VALUES (?,?,?,?,?,?,?,?,?)',(uid,body.name.strip(),email,ph,salt,body.phone.strip(),body.address.strip(),time.time(),'customer'))
 c.commit();c.close();token=create_session(uid)
 return {'token':token,'id':uid,'name':body.name.strip(),'email':email,'phone':body.phone.strip(),'address':body.address.strip(),'ugatu_role':'customer'}
...
def profile(body:ProfileBody):
 ...
 if not r:raise HTTPException(401,'Invalid or expired session')
 name=body.name.strip() or r['name'];c=get_conn();c.execute('UPDATE users SET name=?,phone=?,address=? WHERE id=?',(name,body.phone.strip(),body.address.strip(),r['id']));c.commit();c.close();return {'id':r['id'],'name':name,'email':r['email'],'phone':body.phone.strip(),'address':body.address.strip(),'ugatu_role':r['ugatu_role'] or 'customer'}
...
@router.get('/auth/ugatu/roles')
def ugatu_roles(x_admin_passcode:str=Header(default='')):
 if x_admin_passcode!=ADMIN_PASSCODE:raise HTTPException(401,'Master admin passcode required')
 return {'roles':sorted(ROLE_RULES.keys())}

@router.post('/auth/ugatu/role')
def set_ugatu_role(body:AdminUGATURoleBody,x_admin_passcode:str=Header(default='')):
 if x_admin_passcode!=ADMIN_PASSCODE:raise HTTPException(401,'Master admin passcode required')
 email=body.email.strip().lower();role=normalize_role(body.role)
 if role not in ROLE_RULES:raise HTTPException(400,'Unknown UGATU role')
 c=get_conn();r=c.execute('SELECT id FROM users WHERE email=?',(email,)).fetchone()
 if not r:c.close();raise HTTPException(404,'User not found')
 c.execute('UPDATE users SET ugatu_role=? WHERE id=?',(role,r['id']));c.execute('DELETE FROM sessions WHERE user_id=?',(r['id'],));c.commit();c.close()
 return {'status':'ugatu_role_updated','email':email,'ugatu_role':role,'sessions_revoked':True}
